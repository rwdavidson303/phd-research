...
1. **Codifying value for money as the "core rule"** — rather than one option among several — sends an unambiguous signal to the workforce and the market. The US could achieve a similar effect by establishing best-value tradeoff as the default evaluation method in the FAR, requiring written justification for LPTA use.
